...
export default function PedidosPage() {
    ...
    const [pedidoEditando, setPedidoEditando] = useState<pedidosService.PedidoResponse | null>(null);

    // Estados de datos
    const [pedidos, setPedidos] = useState<pedidosService.PedidoResponse[]>([]);
    ...
    // Cargar productos y pedidos al montar
    useEffect(() => {
        cargarProductos();
        cargarPedidos();
    }, []);
    ...
    const validarFormulario = (): string | null => {
        ...
        if (productosSeleccionados.length === 0) return 'Debe seleccionar al menos un producto';

        for (const producto of productosSeleccionados) {
            if (!producto.cantidadMin || parseInt(producto.cantidadMin) <= 0) {
                return `La cantidad mínima de "${producto.nombreProducto}" debe ser mayor a 0`;
            }

            if (producto.cantidadMax && parseInt(producto.cantidadMax) > 0) {
                if (parseInt(producto.cantidadMax) < parseInt(producto.cantidadMin)) {
                    return `La cantidad máxima de "${producto.nombreProducto}" debe ser mayor o igual a la mínima`;
                }
            }
        }
        ...
        try {
            ...
            const productosSeleccionados = productosFormulario
                ...
                .map(p => ({
                    ...
                    ...(p.cantidadMax && parseInt(p.cantidadMax) > 0
                        ? { cantidadMax: parseInt(p.cantidadMax) }
                        : {})
                }));

            const datos: pedidosService.CrearPedidoRequest = {
                productos: productosSeleccionados,
                fechaCierre: fechaCierre
            };

            await pedidosService.crearPedido(user.idUsuario, datos);
    ...
    const handleActivarPedido = async (idPedido: number) => {
        if (!confirm('¿Estás seguro de activar este pedido?')) return;

        try {
            setCargando(true);
            await pedidosService.actualizarEstadoPedido(idPedido, 'ACT');
    ...
    const handleAbrirEdicion = (pedido: pedidosService.PedidoResponse) => {
    ...
    const handleAgregarProducto = async (idProducto: number, cantidadMin: number, cantidadMax?: number) => {
        ...
        try {
            setCargando(true);
            const producto: pedidosService.ProductoPedidoRequest = {
                idProducto,
                cantidadMin,
                ...(cantidadMax && cantidadMax > 0 ? { cantidadMax } : {})
            };

            const pedidoActualizado = await pedidosService.agregarProductoAlPedido(pedidoEditando.idPedido, producto);
    ...
    const handleModificarCantidades = async (idProducto: number, cantidadMin?: number, cantidadMax?: number) => {
        ...
        try {
            setCargando(true);
            const datos: Partial<pedidosService.ProductoPedidoRequest> = {};
            if (cantidadMin !== undefined) datos.cantidadMin = cantidadMin;
            if (cantidadMax !== undefined) datos.cantidadMax = cantidadMax;

            const pedidoActualizado = await pedidosService.modificarProductoDelPedido(
    ...
    const handleEliminarProducto = async (idProducto: number) => {
        ...
        try {
            setCargando(true);
            const pedidoActualizado = await pedidosService.eliminarProductoDelPedido(pedidoEditando.idPedido, idProducto);
            setPedidoEditando(pedidoActualizado);
                                    ...
                                    <div className="pedido-header">
                                        ...
                                        <div className="pedido-fechas">
                                            <div className="pedido-fecha-item">
                                                ...
                                                <span className="fecha-valor">{pedido.fechaCreado}</span>
                                            </div>
                                            <div className="pedido-fecha-item">
                                                <span className="fecha-label">Cierre:</span>
                                                <span className="fecha-valor">{pedido.fechaCierre}</span>
                                            </div>
                                        </div>
                                    </div>
